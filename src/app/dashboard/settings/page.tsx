...
function Toast({ message, type }: { message: string; type: "success" | "error" }) {
  return (
    <motion.div
      ...
      className="fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2.5 px-5 py-3 rounded-full border text-sm z-50"
      style={{
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-md border border-[#2a1010] rounded-2xl p-6"
      ...
      >
        ...
        <div className="flex gap-3">
          ...
          <motion.button
            whileHover={{ scale: typed === "DELETE" ? 1.02 : 1 }}
...
export default function SettingsPage() {
  ...
  const [form, setForm] = useState({ full_name: "", profession: "", wallet_address: "", country: "Nigeria" });
        ...
        <div className="flex-1 px-5 md:px-8 py-6 max-w-2xl mx-auto w-full space-y-5">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <form onSubmit={handleSaveProfile} className="px-6 py-5 space-y-4">
              ...
              <Field label="Solana Wallet Address (optional)" name="wallet_address"
              ...
              <div>
                ...
                <div className="flex items-center gap-2 bg-[#0d0d0d] border border-[#111] rounded-xl px-4 py-3">
                  <p className="flex-1 text-[#333] text-sm truncate"
                    style={{ fontFamily: "'DM Mono', monospace" }}>{email}</p>
                  <span className="text-[9px] tracking-widest text-[#222] border border-[#1a1a1a] px-2 py-0.5 rounded-full">
                    Read only
                  </span>
                </div>
              </div>
